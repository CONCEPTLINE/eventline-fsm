-- 201_location_stats_types_fix.sql
-- ============================================================
-- Fix: get_location_stats warf "structure of query does not match
-- function result type" beim Aufruf. Postgres ist bei RETURNS TABLE
-- + SECURITY DEFINER + PL/pgSQL strict — die Precision-Suffixe der
-- numeric-Spalten (numeric(8,2), numeric(12,2)) werden im Return-Type
-- ignoriert, aber trotzdem strict verglichen und schlagen fehl wenn
-- die zurueckgegebenen Werte nicht exakt matchen (z.B. wenn ein
-- Subquery-Ausdruck ein unqualifiziertes numeric liefert).
-- Auch integer vs int-Nuancen und double-precision aus extract()
-- koennen betroffen sein.
--
-- Fix: alle Return-Types auf unqualifiziertes numeric / bigint /
-- integer setzen. Werte in der SELECT-Zeile werden explizit gecastet.
-- ============================================================
